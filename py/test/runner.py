# MixpanelLexiconSchemas SDK test runner
...
class MixpanelLexiconSchemasTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            MixpanelLexiconSchemasTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = MixpanelLexiconSchemasTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = MixpanelLexiconSchemasTestRunner.getenv("MIXPANEL_LEXICON_SCHEMAS_TEST_LIVE")
        override = MixpanelLexiconSchemasTestRunner.getenv("MIXPANEL_LEXICON_SCHEMAS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = MixpanelLexiconSchemasTestRunner.getenv(key)
        ...
        explain = MixpanelLexiconSchemasTestRunner.getenv("MIXPANEL_LEXICON_SCHEMAS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["MIXPANEL_LEXICON_SCHEMAS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if MixpanelLexiconSchemasTestRunner._test_control is not None:
            return MixpanelLexiconSchemasTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                MixpanelLexiconSchemasTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            MixpanelLexiconSchemasTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = MixpanelLexiconSchemasTestRunner.load_test_control()
...
def load_env_local():
    MixpanelLexiconSchemasTestRunner.load_env_local()


def env_override(m):
    return MixpanelLexiconSchemasTestRunner.env_override(m)


def entity_data(v):
    return MixpanelLexiconSchemasTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return MixpanelLexiconSchemasTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return MixpanelLexiconSchemasTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return MixpanelLexiconSchemasTestRunner.load_test_control()


def live_client_options():
    return MixpanelLexiconSchemasTestRunner.live_client_options()


def live_delay_ms():
    return MixpanelLexiconSchemasTestRunner.live_delay_ms()
