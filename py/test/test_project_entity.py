    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set MIXPANEL_LEXICON_SCHEMAS_TEST_PROJECT_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        project_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.project")))
        project_ref01_data = None
        if len(project_ref01_data_raw) > 0:
...
def _project_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "MIXPANEL_LEXICON_SCHEMAS_TEST_PROJECT_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("MIXPANEL_LEXICON_SCHEMAS_TEST_PROJECT_ENTID"))
