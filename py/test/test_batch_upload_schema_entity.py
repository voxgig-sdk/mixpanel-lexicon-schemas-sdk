...
class TestBatchUploadSchemaEntity:

    def test_should_create_instance(self):
        testsdk = MixpanelLexiconSchemasSDK.test(None, None)
        ent = testsdk.BatchUploadSchema(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _batch_upload_schema_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "batch_upload_schema." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set MIXPANEL_LEXICON_SCHEMAS_TEST_BATCH_UPLOAD_SCHEMA_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        batch_upload_schema_ref01_ent = client.BatchUploadSchema(None)
        ...
        batch_upload_schema_ref01_data = helpers.to_map(runner.entity_data(batch_upload_schema_ref01_ent.create(batch_upload_schema_ref01_data, None)))
        assert batch_upload_schema_ref01_data is not None



def _batch_upload_schema_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/batch_upload_schema/BatchUploadSchemaTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "MIXPANEL_LEXICON_SCHEMAS_TEST_BATCH_UPLOAD_SCHEMA_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("MIXPANEL_LEXICON_SCHEMAS_TEST_BATCH_UPLOAD_SCHEMA_ENTID"))
