...
describe("ProjectEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set MIXPANEL_LEXICON_SCHEMAS_TEST_PROJECT_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local project_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.project")))
    local project_ref01_data = nil
    if #project_ref01_data_raw > 0 then
      project_ref01_data = helpers.to_map(project_ref01_data_raw[1][2])
    end
...
function project_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("MIXPANEL_LEXICON_SCHEMAS_TEST_PROJECT_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["MIXPANEL_LEXICON_SCHEMAS_TEST_PROJECT_ENTID"])
