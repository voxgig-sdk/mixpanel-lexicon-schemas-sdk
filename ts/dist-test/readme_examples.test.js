...
const __1 = require("..");
const SDK_NAME = 'MixpanelLexiconSchemasSDK';
...
function isRunnable(code) {
    return (/new\s+MixpanelLexiconSchemasSDK\b/.test(code) ||
        /\bMixpanelLexiconSchemasSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+MixpanelLexiconSchemasSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/MixpanelLexiconSchemasSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
