...
// MixpanelLexiconSchemas SDK BatchUploadSchema entity
...
class BatchUploadSchemaEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "batch_upload_schema";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new BatchUploadSchemaEntity($this->_client, $opts);
    }

    /**
     * @param BatchUploadSchema|array $args BatchUploadSchema data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = MixpanelLexiconSchemasHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return BatchUploadSchema|array The current BatchUploadSchema data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of BatchUploadSchema fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = MixpanelLexiconSchemasHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of BatchUploadSchema fields).
     */
    ...
    /**
     * Create a new BatchUploadSchema.
     *
     * @param BatchUploadSchemaCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed BatchUploadSchemaCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return BatchUploadSchema|array The created BatchUploadSchema as an assoc-array at the
     *   SDK boundary; throws MixpanelLexiconSchemasError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = MixpanelLexiconSchemasHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
