...
class ProjectEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MIXPANEL_LEXICON_SCHEMAS_TEST_PROJECT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // Bootstrap entity data from existing test data.
        $project_ref01_data_raw = Vs::items(Helpers::to_map(
            Vs::getpath($setup["data"], "existing.project")));
        $project_ref01_data = null;
        if (count($project_ref01_data_raw) > 0) {
            $project_ref01_data = Helpers::to_map($project_ref01_data_raw[0][1]);
        }
...
function project_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("MIXPANEL_LEXICON_SCHEMAS_TEST_PROJECT_ENTID");
