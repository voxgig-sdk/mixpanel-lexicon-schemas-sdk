...
{
    public function test_direct_list_schema(): void
    {
        $setup = schema_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-schema", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["project01"] as $_liveKey) {
...
function schema_direct_setup($mockres)
{
    ...
    if ($live) {
        ...
        $client = new MixpanelLexiconSchemasSDK($merged_opts);
        return [
            "client" => $client,
            "calls" => $calls,
            "live" => true,
