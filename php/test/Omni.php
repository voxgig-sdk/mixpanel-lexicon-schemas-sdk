...
//    only - a typed MixpanelLexiconSchemasContext there would make every ctx
...
final class MixpanelLexiconSchemasOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new MixpanelLexiconSchemasContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new MixpanelLexiconSchemasSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new MixpanelLexiconSchemasResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new MixpanelLexiconSchemasError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new MixpanelLexiconSchemasResponse($clean['response']);
    ...
    public static function err_from_map($m): ?MixpanelLexiconSchemasError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new MixpanelLexiconSchemasError(is_string($code) ? $code : '', $msg);
    }
