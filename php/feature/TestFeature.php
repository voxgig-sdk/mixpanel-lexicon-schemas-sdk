...
// MixpanelLexiconSchemas SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class MixpanelLexiconSchemasTestFeature extends MixpanelLexiconSchemasBaseFeature
{
    ...
    public function init(MixpanelLexiconSchemasContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (MixpanelLexiconSchemasContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = MixpanelLexiconSchemasParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (MixpanelLexiconSchemasContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(MixpanelLexiconSchemasContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = MixpanelLexiconSchemasParam::call($ctx, $k);
