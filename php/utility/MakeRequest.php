...
class MixpanelLexiconSchemasMakeRequest
{
    public static function call(MixpanelLexiconSchemasContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new MixpanelLexiconSchemasResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new MixpanelLexiconSchemasResponse($fetched);
        } else {
