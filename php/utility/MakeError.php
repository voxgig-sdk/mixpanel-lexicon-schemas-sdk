...
class MixpanelLexiconSchemasMakeError
{
    public static function call(?MixpanelLexiconSchemasContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new MixpanelLexiconSchemasContext([], null);
        }
        $op = $ctx->op ?? new MixpanelLexiconSchemasOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new MixpanelLexiconSchemasResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof MixpanelLexiconSchemasError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new MixpanelLexiconSchemasError('', $msg, $ctx);
        ...
        if ($err instanceof MixpanelLexiconSchemasError) {
