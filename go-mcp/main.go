// Command MixpanelLexiconSchemas-mcp is an MCP server exposing the MixpanelLexiconSchemas
// SDK as MCP tools.
//
// Two tools are registered:
//
//	MixpanelLexiconSchemas_list  — list records of an entity
//	MixpanelLexiconSchemas_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: MIXPANEL_LEXICON_SCHEMAS_APIKEY carries the API key and
	// MIXPANEL_LEXICON_SCHEMAS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("MIXPANEL_LEXICON_SCHEMAS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("MIXPANEL_LEXICON_SCHEMAS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewMixpanelLexiconSchemasSDK(opts)
