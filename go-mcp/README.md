# mixpanel-lexicon-schemas-mcp

[MCP](https://modelcontextprotocol.io) server exposing the MixpanelLexiconSchemas SDK as
two agent tools — `mixpanel-lexicon-schemas_list` and `mixpanel-lexicon-schemas_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/mixpanel-lexicon-schemas-mcp)
make build

# 2. Provide credentials via the environment
export MIXPANEL_LEXICON_SCHEMAS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user mixpanel-lexicon-schemas \
  -- /absolute/path/to/mixpanel-lexicon-schemas-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./mixpanel-lexicon-schemas-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// mixpanel-lexicon-schemas_list: first page of records
{ "entity": "schema" }
{ "entity": "schema", "query": { } }

// mixpanel-lexicon-schemas_load: one record by id
{ "entity": "schema", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/mixpanel-lexicon-schemas-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user mixpanel-lexicon-schemas \
     -- "$PWD"/dist/*/mixpanel-lexicon-schemas-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `mixpanel-lexicon-schemas_list` and `mixpanel-lexicon-schemas_load` tools now appear
   in new sessions. Ask the agent to *"list schema using mixpanel-lexicon-schemas"*
   and it calls `mixpanel-lexicon-schemas_list` with `{"entity":"schema"}`.
...
```sh
export MIXPANEL_LEXICON_SCHEMAS_APIKEY=sk_live_xxx            # API key
export MIXPANEL_LEXICON_SCHEMAS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `mixpanel-lexicon-schemas_list` tool
...
### Call the `mixpanel-lexicon-schemas_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `mixpanel-lexicon-schemas_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `mixpanel-lexicon-schemas_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `MIXPANEL_LEXICON_SCHEMAS_APIKEY` | API key sent with every request. |
| `MIXPANEL_LEXICON_SCHEMAS_BASE` | Optional override of the API base URL. |
...
```sh
./mixpanel-lexicon-schemas-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"mixpanel-lexicon-schemas_load","arguments":{"entity":"schema","query":{"id":1}}}}'
```
