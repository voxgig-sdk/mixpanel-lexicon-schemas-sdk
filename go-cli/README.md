# mixpanel-lexicon-schemas-cli

boru-driven command-line client **and** interactive REPL for the MixpanelLexiconSchemas
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/mixpanel-lexicon-schemas-cli)
make build

# 2. See usage (words, entities, env vars)
./mixpanel-lexicon-schemas-cli --help
...
MIXPANEL_LEXICON_SCHEMAS_BASE=https://api.example.com ./mixpanel-lexicon-schemas-cli --help

# 6. No arguments -> interactive REPL
./mixpanel-lexicon-schemas-cli
mixpanel-lexicon-schemas> /help
mixpanel-lexicon-schemas> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/mixpanel-lexicon-schemas-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/mixpanel-lexicon-schemas-cli --help
   ```
...
Run with no arguments to open a REPL (prompt `mixpanel-lexicon-schemas>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `batch_upload_schema`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/mixpanel-lexicon-schemas-cli`. |
...
(`./mixpanel-lexicon-schemas-cli <expr>`) and interactively (the REPL), and expressions compose the
