...
// MixpanelLexiconSchemas SDK feature test
//
...
class FtCtrl extends MixpanelLexiconSchemasControl
{
...
class FtClient
{
    ...
    private MixpanelLexiconSchemasUtility $utility;

    public function __construct(MixpanelLexiconSchemasUtility $utility, array $options)
    {
    ...
    public function get_utility(): MixpanelLexiconSchemasUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MixpanelLexiconSchemasContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public MixpanelLexiconSchemasContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (MixpanelLexiconSchemasContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MixpanelLexiconSchemasConfig::make_feature($name);
    ...
    private static function buildUrl(MixpanelLexiconSchemasSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MixpanelLexiconSchemasError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MixpanelLexiconSchemasResponse($fetched);
        }
    ...
    private function fail(MixpanelLexiconSchemasContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MixpanelLexiconSchemasContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
