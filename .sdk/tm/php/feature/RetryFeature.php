...
// MixpanelLexiconSchemas SDK retry feature
...
class MixpanelLexiconSchemasRetryFeature extends MixpanelLexiconSchemasBaseFeature
{
    ...
    public function init(MixpanelLexiconSchemasContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (MixpanelLexiconSchemasContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(MixpanelLexiconSchemasContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(MixpanelLexiconSchemasContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
