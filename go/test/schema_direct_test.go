...
func TestSchemaDirect(t *testing.T) {
	t.Run("direct-list-schema", func(t *testing.T) {
		setup := schemaDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-schema", _mode); _shouldSkip {
			if _reason == "" {
...
type schemaDirectSetupResult struct {
	client *sdk.MixpanelLexiconSchemasSDK
...
func schemaDirectSetup(mockres any) *schemaDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"MIXPANEL_LEXICON_SCHEMAS_TEST_SCHEMA_ENTID": map[string]any{},
	...
	if live {
		...
		client := sdk.NewMixpanelLexiconSchemasSDK(mergedOpts)

		idmap := map[string]any{}
		if entidRaw, ok := env["MIXPANEL_LEXICON_SCHEMAS_TEST_SCHEMA_ENTID"]; ok {
